...
class TaskBasicTests: XCTestCase {
    ...
    private let executeQueue = DispatchQueue(label: "com.alecrim.AlecrimAsyncKitTests.Execute", attributes: .concurrent)
    ...
    func testImmediateValue() {
        ...
        do {
            let value = try task.execute(on: self.executeQueue)
            XCTAssert(value == 10)
        }
        catch {
            XCTAssert(false)
    ...
    func testImmediateError() {
        ...
        do {
            let _ = try task.execute(on: self.executeQueue)
            XCTAssert(false)
        }
        catch {
    ...
    func testNonFailableImmediateValue() {
        ...
        let value = task.execute(on: self.executeQueue)
        XCTAssert(value == 10)
    }
